...
export function CustomSolutionTab({ onRequestCustomSolution }: CustomSolutionTabProps) {
  ...
  const handleSubmitConfiguration = async () => {
    if (!projectDescription.trim()) {
      toast.error('Please describe your automation needs');
      return;
    }
    ...
    try {
      // Filter out empty integration strings
      const filteredSystems = integrations.filter(system => system.trim() !== '');
      
      // Insert into Supabase
      const { error } = await supabase.from('custom_automation_requests').insert({
        user_id: user?.id || null,
        system_description: projectDescription,
        current_systems: filteredSystems,
        requirements: [], // Empty array as default
        industry: 'Technology', // Default value
        timeline: '3-6 months', // Default value
        budget: budget,
        name: user?.user_metadata?.first_name || 'Anonymous',
        email: user?.email || 'anonymous@example.com',
        company: user?.user_metadata?.business_name || null,
        phone: user?.user_metadata?.phone || null
      });
      
      if (error) throw error;
      
      toast.success('Your automation configuration has been submitted!');
      ...
      toast.error('Failed to submit your configuration. Please try again.');
    } finally {
            ...
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div className="space-y-8">
                ...
                <div className="mt-8 flex flex-col gap-4">
                  ...
                  <button
                    onClick={onRequestCustomSolution}
                    className="w-full bg-gray-800 hover:bg-gray-700 text-white px-8 py-4 rounded-lg font-semibold transition-all duration-300 flex items-center justify-center gap-2"
                  >
